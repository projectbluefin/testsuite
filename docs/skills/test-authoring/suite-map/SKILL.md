...
527 scenarios across 72 feature files: 414 active, 0 quarantined, 113 `@future`/`@pending`/`@hardware_blocked`
...
| vanilla-gnome | 14 | 14 | 0 | 0 | Baseline GNOME Shell parity check; runs on any GNOME image; includes an informational ShellVersion canary scenario (#826) that prints the running org.gnome.Shell ShellVersion to the run log and never gates the run |
