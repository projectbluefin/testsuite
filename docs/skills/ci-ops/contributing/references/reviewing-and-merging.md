...
**GHA CI green → human approval → merge queue.**

### The gate to apply

1. **GitHub Actions CI green** — the checks below.
...
| Check | Workflow | Trigger | Ruleset-required |
|---|---|---|---|
| `Lint & syntax` | `pr-validate.yml` | `pull_request`, `merge_group`, `push: main` | yes |
| `Behave dry-run` | `pr-validate.yml` | same | yes |
| `Quarantine age` | `pr-validate.yml` | same | no |
| `pytest` | `unit-tests.yml` | `pull_request`, `merge_group`, `push: main` | yes |
| `docs-validate` | `docs-validate.yml` | `pull_request`, `merge_group`, `push: main` | no |

The three marked required are the status checks configured on the
`main — merge queue` repository **ruleset** (which also enables the merge
queue, squash method, and `ALLGREEN` grouping). The other two run on every PR
and should be green before you ask for approval, but the ruleset does not
block on them. `gh api repos/projectbluefin/testsuite/branches/main/protection`
...
### CI does not boot a VM

Lint, behave dry-run, quarantine age, pytest, and docs-validate verify syntax,
step-name resolution, doc structure, and helper unit behaviour only. Real-VM
regressions — GNOME Shell/AT-SPI timing, GDM state, bootc upgrade/rollback,
oomd kills — are invisible to all of them, and **there is no pre-merge real-VM
...
Verify a gate, reporter, or check by observing a real signal end to end. Never
conclude one works because a fix for it merged. Reporting is a leg of the
pipeline that no upstream test exercises — an unauthenticated request or a
rejected payload fails silently from the PR's point of view. This is the same
failure class as a test suite that reports green while silently skipping every
scenario.
